...
//            id_hd,Username,name,expiry_date,quantity,unit_rate
            ...
            String q2 = "select hmisdb.med_issued.pre_id as pre_id, hmisdb.medicine.name as med,"
                    ...
                    + "hmisdb.medicine.unit_rate as rate\n" +
                    "from hmisdb.med_issued \n" +
                    "left outer join hmisdb.medicine on hmisdb.medicine.med_id=hmisdb.med_issued.med_id;";
            ResultSet rest1 = obj.stm.executeQuery(q2);
                
            while(rest1.next()){
                qty = Integer.parseInt(rest1.getString("qty"));
                MRP = Integer.parseInt(rest1.getString("rate"));
                Amount = MRP*qty;
                Total+=Amount;
                y[i][j++] = ++Sr_n +"";
                y[i][j++] = rest1.getString("pre_id");
                y[i][j++] = rest1.getString("med"); // medicinae_name
                y[i][j++] = rest1.getString("exp");// exp date
                y[i][j++] = rest1.getString("qty");
                y[i][j++] = rest1.getString("rate");
                y[i][j++] = Amount+"";//Amount
                i++;
                j=0;
