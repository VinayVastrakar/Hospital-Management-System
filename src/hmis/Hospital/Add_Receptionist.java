...
public class Add_Receptionist implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae){
         ...
         if(ae.getSource()==bt1){
             ...
             Random r= new Random();
             String rec_id = ""+Math.abs(r.nextInt()%100000);
             
             try{
                 ConnectionClass obj = new ConnectionClass();
                 String q = "insert into Receptionist values('"+rec_id+"','"+name+"','"+username+"','"+password+"','"
                         +email+"','"+fName+"','"+phone+"','"+qualification+"','"+city+"','"+gender+"','"+blood+"','"+joining_date
                         +"','"+age+"','"+address+"','"+dob+"')";
                 obj.stm.executeUpdate(q);
                 JOptionPane.showMessageDialog(null, "Receptionist Details Successfull Inserted");
                 f.setVisible(false);
             }
             catch(Exception e){
                 e.printStackTrace();
